...
class SeesawPCBLoss(nn.Cell):
    ...
    def construct(self,
        ...
        if pos_inds.sum() > 0:
            pred_fg_distri = ops.softmax(cls_score_classes[pos_inds, :self.num_classes])
            fg_confusion_matrix_tmp = ops.zeros_like(self.fg_confusion_matrix).scatter_add_(labels[pos_inds].view(-1, 1).repeat(1, self.num_classes),
                                                                                            pred_fg_distri)
            ...
            num_inst_cnt_tmp = ops.zeros_like(self.num_inst_cnt).scatter_add(labels[pos_inds],
